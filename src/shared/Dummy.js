// 키값은 아마 다를거에요 스웨거허브가 요금제한으로 끝나서 확인불가 ㅠ
export const homeDummy = [
  {
    인기작품: [
      {
        category: "회화",
        postId: "asdfasdf",
        postTitle: "회화 팔아요",
        price: 10000,
        transaction: "직거래",
        changeAddress: "",
        markupCnt: 24,
        createdAt: "2022-04-26 12:00:00",
        size: "29.7 x 42cm",
        content: "작품에 대한 설명이 들억나ㅡㄴ 영역입니다",
        imageUrl: [
          "https://cdn.clien.net/web/api/file/F01/12355532/2e10d6d02e7df0.jpg?w=780&h=30000",
          "https://img.hankyung.com/photo/202108/99.26501439.1-1200x.jpg",
          "http://via.placeholder.com/400x300",
        ],
        user: {
          userId: "user444",
          nickname: "판매자777",
          profileUrl:
            "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
          address: "고양시 덕양구",
        },
      },
      {
        category: "조각",
        postId: "asdfasdf123",
        postTitle: "다비드 조각상",
        price: 2000000,
        transaction: "직거래",
        changeAddress: "",
        markupCnt: 10,
        createdAt: "2022-04-27 12:00:00",
        imageUrl: ["http://via.placeholder.com/400x300"],
        size: "29.7 x 42cm",
        content: "작품에 대한 설명이 들억나ㅡㄴ 영역입니다",
        user: {
          userId: "user999",
          nickname: "조각상헌터",
          profileUrl:
            "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
          address: "동두천 어쩌구",
        },
      },
      {
        category: "패션/주얼리",
        postId: "qwerqwer",
        postTitle: "패션주얼리",
        price: 30000,
        transaction: "직거래",
        changeAddress: "",
        markupCnt: 8,
        createdAt: "2022-04-28 12:00:00",
        size: "29.7 x 42cm",
        content: "작품에 대한 설명이 들억나ㅡㄴ 영역입니다",
        imageUrl: ["https://t1.daumcdn.net/cfile/tistory/24283C3858F778CA2E"],
        user: {
          userId: "user123",
          nickname: "판매자1",
          profileUrl:
            "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
          address: "광주 어디",
        },
      },
      {
        category: "기타",
        postId: "qwer1234",
        postTitle: "기타 팔아요",
        price: 2000,
        transaction: "직거래",
        changeAddress: "",
        markupCnt: 4,
        createdAt: "2022-04-28 17:30:24",
        size: "29.7 x 42cm",
        content: "작품에 대한 설명이 들억나ㅡㄴ 영역입니다",
        imageUrl: [
          "https://img.hankyung.com/photo/202108/99.26501439.1-1200x.jpg",
          "https://cdn.clien.net/web/api/file/F01/12355532/2e10d6d02e7df0.jpg?w=780&h=30000",
        ],
        user: {
          userId: "user123",
          nickname: "판매자1",
          profileUrl:
            "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
          address: "대전 유성구",
        },
      },
    ],
  },
  {
    주목작가: [
      {
        userId: "user1234",
        nickname: "피카소",
        profileUrl:
          "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
        address: "서울 중랑구",
      },
      {
        userId: "qwerqwerrrr",
        nickname: "고흐",
        profileUrl:
          "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
        address: "고양시 어디구",
      },
      {
        userId: "tttttttttt",
        nickname: "렘브란트",
        profileUrl:
          "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
        address: "분당 서현구",
      },
      {
        userId: "user123",
        nickname: "판매자1",
        profileUrl:
          "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
        address: "대전 유성구",
      },
    ],
  },
  {
    후기: [
      {
        category: "도자",
        reviewId: 1,
        userId: "abc123",
        nickname: "조각상헌터",
        createdAt: "2022-04-25 HH:mm:ss",
        reviewTitle: "너무 만족해요",
        reviewContent: "이분 나중에 꼭 성공하실거같아요 센스 있으신거같네요",
        likeCnt: 24,
        imageUrl: [
          "https://post-phinf.pstatic.net/MjAxNzA2MjNfMjQg/MDAxNDk4MTk2MDY3OTEz.qtfMx30zMABnlyZWWFHSwH12EqX8okijpFTHpHIcc1Eg.NJG-cvNd_0FvE6HfNhKeBIKqA9mPgJwdJm_doc8-8T0g.PNG/170622_%EB%A6%AC%EB%B9%99%ED%8C%90%EC%BB%A8%ED%85%90%EC%B8%A0_2.png?type=w1200",
          "asdfasdf",
          "asdfasdfasdf",
        ],
      },
      {
        category: "도자",
        reviewId: 2,
        userId: "abc123",
        nickname: "조각상헌터",
        createdAt: "2022-04-25 HH:mm:ss",
        reviewTitle: "너무 만족해요",
        reviewContent: "이분 나중에 꼭 성공하실거같아요 센스 있으신거같네요",
        likeCnt: 24,
        imageUrl: [
          "https://post-phinf.pstatic.net/MjAxNzA2MjNfMjQg/MDAxNDk4MTk2MDY3OTEz.qtfMx30zMABnlyZWWFHSwH12EqX8okijpFTHpHIcc1Eg.NJG-cvNd_0FvE6HfNhKeBIKqA9mPgJwdJm_doc8-8T0g.PNG/170622_%EB%A6%AC%EB%B9%99%ED%8C%90%EC%BB%A8%ED%85%90%EC%B8%A0_2.png?type=w1200",
          "asdfasdf",
          "asdfasdfasdf",
        ],
      },
      {
        category: "도자",
        reviewId: 3,
        userId: "abc123",
        nickname: "조각상헌터",
        createdAt: "2022-04-25 HH:mm:ss",
        reviewTitle: "너무 만족해요",
        reviewContent: "이분 나중에 꼭 성공하실거같아요 센스 있으신거같네요",
        likeCnt: 24,
        imageUrl: [
          "https://post-phinf.pstatic.net/MjAxNzA2MjNfMjQg/MDAxNDk4MTk2MDY3OTEz.qtfMx30zMABnlyZWWFHSwH12EqX8okijpFTHpHIcc1Eg.NJG-cvNd_0FvE6HfNhKeBIKqA9mPgJwdJm_doc8-8T0g.PNG/170622_%EB%A6%AC%EB%B9%99%ED%8C%90%EC%BB%A8%ED%85%90%EC%B8%A0_2.png?type=w1200",
          "asdfasdf",
          "asdfasdfasdf",
        ],
      },
      {
        category: "도자",
        reviewId: 4,
        userId: "abc123",
        nickname: "조각상헌터",
        createdAt: "2022-04-25 HH:mm:ss",
        reviewTitle: "너무 만족해요",
        reviewContent: "이분 나중에 꼭 성공하실거같아요 센스 있으신거같네요",
        likeCnt: 24,
        imageUrl: [
          "https://post-phinf.pstatic.net/MjAxNzA2MjNfMjQg/MDAxNDk4MTk2MDY3OTEz.qtfMx30zMABnlyZWWFHSwH12EqX8okijpFTHpHIcc1Eg.NJG-cvNd_0FvE6HfNhKeBIKqA9mPgJwdJm_doc8-8T0g.PNG/170622_%EB%A6%AC%EB%B9%99%ED%8C%90%EC%BB%A8%ED%85%90%EC%B8%A0_2.png?type=w1200",
          "asdfasdf",
          "asdfasdfasdf",
        ],
      },
    ],
  },
];

export const storeDummy = [
  {
    category: "기타",
    postId: "asdfasdf",
    postTitle: "기타 팔아요",
    price: 2000,
    transaction: "직거래",
    changeAddress: "",
    markupCnt: 0,
    createdAt: "2022-04-28 12:00:00",
    size: "29.7 x 42cm",
    content: "작품에 대한 설명이 들억나ㅡㄴ 영역입니다",
    imageUrl: [
      "https://cdn.clien.net/web/api/file/F01/12355532/2e10d6d02e7df0.jpg?w=780&h=30000",
      "https://img.hankyung.com/photo/202108/99.26501439.1-1200x.jpg",
      "http://via.placeholder.com/400x300",
    ],
    user: {
      userId: "user123",
      nickname: "판매자1",
      profileUrl:
        "https://pbs.twimg.com/profile_images/1374979417915547648/vKspl9Et_400x400.jpg",
      address: "대전 유성구",
    },
  },
  {
    category: "회화",
    postId: "qwerqwer",
    postTitle: "그림 팔아요",
    price: 0,
    transaction: "택배",
    changeAddress: "",
    markupCnt: 3,
    size: "29.7 x 42cm",
    content: "작품에 대한 설명이 들억나ㅡㄴ 영역입니다",
    imageUrl: [
      "https://img.hankyung.com/photo/202108/99.26501439.1-1200x.jpg",
      "http://via.placeholder.com/400x300",
    ],
    user: {
      userId: "user444",
      nickname: "닉네임99",
      profileUrl:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSi8sTwwX22p9l0nz43jBSvHw4dlqMxU7XFDg&usqp=CAU",
      address: "서울 동작구",
    },
  },
  {
    category: "조각",
    postId: "postIdforchat",
    postTitle: "채팅용",
    price: 2000,
    transaction: "직거래",
    changeAddress: "",
    markupCnt: 1,
    createdAt: "2022-04-28 12:00:00",
    size: "29.7 x 42cm",
    content: "채팅용",
    imageUrl: [
      "https://cdn.clien.net/web/api/file/F01/12355532/2e10d6d02e7df0.jpg?w=780&h=30000",
      "https://img.hankyung.com/photo/202108/99.26501439.1-1200x.jpg",
      "http://via.placeholder.com/400x300",
    ],
    user: {
      nickname: "이한울",
      profileUrl:
        "http://k.kakaocdn.net/dn/blXfTb/btq5FhvAxNd/S37fBUxE0CUskeQNo4mA4K/img_640x640.jpg",
      provider: "kakao",
      userId: "2222423044",
      address: "대구 달성군",
    },
  },
];
